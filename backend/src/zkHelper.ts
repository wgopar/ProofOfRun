...
import { getVerifierContract, getCachedSigner } from './contractHelper.js';
...
export async function submitProof(data: proof) {
    ...
    logger.info("Verifying proof on contract at: " + contract.address);
    const contractResponse = await contract.connect(signer).verifyProof(argv[0], argv[1], argv[2], argv[3]);
